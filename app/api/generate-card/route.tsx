...
function buildCard(params: {
  ...
  const { fullName, formattedPinyin, formattedMeaning, luckyLine1, luckyLine2, bgSrc, zodiacSrc, watermark } = params;
  ...
  return (
    ...
    >
      {/* 背景图 */}
      {/* @ts-ignore */}
      <img src={bgSrc} style={{ position: 'absolute', width: '1080px', height: '1080px', objectFit: 'cover' }} />
      ...
      <div style={{
        ...
        {/* 生肖圆形图 */}
        <div style={{ width: '105px', height: '105px', borderRadius: '50%', overflow: 'hidden', marginTop: '30px', display: 'flex' }}>
          {/* @ts-ignore */}
      ...
      {/* 底部网站 */}
      <div style={{ position: 'absolute', bottom: '50px', fontSize: '28px', color: 'white', fontFamily: 'serif', zIndex: 5 }}>
        chinanam.online
      </div>
    </div>
  );
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { fullName, pinyin, meaning, luckyPhrase, zodiac, gender, watermark = false } = body;
    ...
    const imageResponse = new ImageResponse(
      buildCard({ fullName, formattedPinyin, formattedMeaning, luckyLine1, luckyLine2, bgSrc, zodiacSrc, watermark }),
    ...
    return new Response(buffer, {
      ...
      headers: {
        ...
        'Content-Disposition': watermark
          ? 'inline'
          : `attachment; filename="${encodeURIComponent(fullName)}_lucky_card.png"`,
